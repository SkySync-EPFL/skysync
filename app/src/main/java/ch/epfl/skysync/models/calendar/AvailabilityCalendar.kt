...
class AvailabilityCalendar: CalendarModel<Availability>() {
    ...
    /**
     * fills the calendar with availabilities for the given interval. Each availability is
     * init with UNSET_ID and MAYBE
    ...
    override fun initForRange(from: LocalDate, to: LocalDate) {
        initForRangeSuper(from, to) {
                                       date, timeSlot -> Availability(UNSET_ID, AvailabilityStatus.MAYBE, timeSlot, date)
        }
    }
