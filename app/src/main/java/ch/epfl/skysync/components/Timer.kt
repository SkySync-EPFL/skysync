...
fun TimerButton(
  ...
  when (flightStage) {
    ...
    FlightStage.ONGOING ->
        if (isPilot) {
          Button(modifier = modifier.testTag("Stop Button"),
              onClick = { onStop() },
              colors = ButtonDefaults.buttonColors(containerColor = lightOrange)) {
    ...
    FlightStage.POST ->
        Button(modifier = modifier.testTag("Clear Button"),
            onClick = { onClear() },
            colors = ButtonDefaults.buttonColors(containerColor = lightOrange)) {
          Text(text = "Exit Flight")
        }
    FlightStage.DISPLAY ->
        Button(modifier = modifier.testTag("Quit Button"),
...
fun TimerButtonPreview(){
    Box(modifier = Modifier.fillMaxSize()){
        TimerButton(
            modifier = Modifier.fillMaxWidth().align(Alignment.BottomCenter).padding(vertical = 10.dp, horizontal = 20.dp),
