...
class UserManagementViewmodelTest {
    ...
    fun setTempUser() {
        composeTestRule.setContent {
            viewModelAdmin = UserManagementViewmodel.createViewModel(repository = repository, userId ="id-admin-1" )
        }
        val firstname = "John"
        val lastname = "Deer"
        val userRole = UserRole.ADMIN
        val email = "john.deer@gmail.com"
        val testTempUser = TempUser(firstname = firstname,
            lastname = lastname,
            userRole = userRole,
            email = email)
        viewModelAdmin.addTempUser(email,userRole,firstname,lastname,null)
        val vmTempUser = viewModelAdmin.tempUsers.value[0]
    ...
    fun loadsCorrectAdmin() {
        ...
        runTest {
            ...
            Assert.assertEquals("id-admin-1", currentUser?.id)
        }
    }
}