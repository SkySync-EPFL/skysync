...
class GroupChatTest {
    ...
    val searchGroup = Quadruple(
        "GroupSearch",
        image,
        "Last message",
        "Last message time"
    )
    val groups = listOf(quad, quad, quad, quad, quad, quad, quad, quad, quad, quad, quad, quad, quad, quad, quad, quad,quad,searchGroup)
    ...
    fun testGroupCardIsDisplayed() {
        for(i in groups.indices) {
            composeTestRule.onNodeWithTag("GroupChatBody")
                .performScrollToNode(
                    hasTestTag("GroupCard$i")
                )
            composeTestRule.onNodeWithTag("GroupCard$i")
                .assertIsDisplayed()
        }
    }
    @Test
